...
# DATE: June 08, 2020
# USAGE: This script is used along with a bash submission script that specifies the chrom.name, par.name. The script takes fulsib family genotypes as the input. 

# Following is the input format for the fulsib family; 
...
# PURPOSE: The code rads in the Fulsib family genotypes for the focal parent iteratively and compiles all the genotypes of the fullsib family for the focal parent including genotypes of other parents.
# It then attempts to correct the genotype errors of parents based on observed vs. expected genotypes of the offspring for a given cross using the fulsib family information for allof the offspring of 
# the focal parent. This is identified as the "Dobzhansky scoring procedure" - REF: https://www.genetics.org/content/genetics/119/2/465.full.pdf
########################################################################################################################################################################################################
rm(list = ls())
library(dplyr)
library(stringr)
...
input.path.truth <- "./genotypes/"
input.path.VQSR <- "./genotypes/VQSR/"
output.path <- "./genotypes/parents/VQSR/"
########################################################################################################################################################################################################
...
if (any(par.name %in% par.name.vec[1:7])) {
  ...
  for (other.name in other.par.names.vec[1:7]) { # other.name <- "6909"
    digit.tab.t <- as.matrix(read.csv(file = paste0(input.path.truth, chrom.name, "_fulsib_genotypes_", par.name, "X", other.name, ".csv"), stringsAsFactors = FALSE, header = TRUE) )
    digit.tab.v <- as.matrix(read.csv(file = paste0(input.path.VQSR, chrom.name, "_fulsib_genotypes_", par.name, "X", other.name, ".csv"), stringsAsFactors = FALSE, header = TRUE) )
    
    # removing markers in VQSR set that are already present in the truth-set. Not all markers in the truth-set are available in the VQSR-set.
    rmv.idx <- which(!(digit.tab.v[,2] %in% digit.tab.t[,2]))
    digit.tab <- digit.tab.v[rmv.idx, ]
    # cat(nrow(digit.tab), "\n")
  ...
  for (other.name in other.par.names.vec[1:7]) { # other.name <- "1863"
    digit.tab.t <- as.matrix(read.csv(file = paste0(input.path.truth, chrom.name, "_fulsib_genotypes_", other.name, "X", par.name, ".csv"), stringsAsFactors = FALSE, header = TRUE) )
    digit.tab.v <- as.matrix(read.csv(file = paste0(input.path.VQSR, chrom.name, "_fulsib_genotypes_", other.name, "X", par.name, ".csv"), stringsAsFactors = FALSE, header = TRUE) )
...
cat(paste0(output.path, chrom.name, "_corrected_genotypes_", par.name, ".csv"), "Successfully completed!")
# } # par.name bracket
# } # chrom.name bracket
########################################################################################################################################################################################################
# END
# Following is the output format of the corrected genotypes for the parent.
