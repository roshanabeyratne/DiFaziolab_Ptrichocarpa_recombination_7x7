...
cat neg_con_CO_le_10k_df.csv | cut -d ',' -f 1,2,3 | tr ',' '\t' | perl -e 'while(<>){chomp; if($_!~/^CHROM/){@var=split; print "$var[0]\t$var[1]\t$var[2]\t1\n";}}' > 10kb_neg_con_subseqfile_CO.txt
perl 24b_bpsubseq_file.pl soft_link_to_Stet14_V2_Male_Reference 10kb_neg_con_subseqfile_CO.txt > neg_con_CO_le_10k.fasta
...
streme --oc le_10kb --objfun de --dna --p filtered_CO_le_10k.fasta --n neg_con_CO_le_10k.fasta
