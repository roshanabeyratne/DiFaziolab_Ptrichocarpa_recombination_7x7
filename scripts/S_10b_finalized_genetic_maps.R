#!/usr/bin/R
setwd("/group/difazio/populus/gatk-7x7-Stet14/")
# setwd("/Users/cabeyrat/ownCloud/BESC/BESC_DATA/7x7/Stettler_14_linkage_mapping")
########################################################################################################################################################################################################
# METADATA: Written by CRA & DMS, for the recombination landscape analysis for 7x7 cross
# DATE: January 01, 2021; Last Modified: September 24, 2020
# USAGE: Input files include genetic maps and table with physical vs. genetic map sizes. The marker names contain the chromosome and marker putative physical position. 

# Following is the input file / genetic map format; 
# 1 Chr06_417767 0
# 1 Chr06_5256 9.99999999868873e-06
# 1 Chr06_685931 0.72899955945013
# 1 Chr06_472891 0.729009559450128
# 1 Chr06_811815 2.18926150399718
# 1 Chr06_804902 2.18927150399718
# 1 Chr06_811828 2.18928150399718
# 1 Chr06_1049955 2.91925669384554
# 1 Chr06_1037834 2.91926669384554
# 1 Chr06_1297578 3.64925286278189
# 1 Chr06_1139116 3.64926286278189
# 1 Chr06_1071037 3.64927286278189

# PURPOSE: Write the finalized genetic maps in the format that Hudson-Alpha requires.
########################################################################################################################################################################################################
rm(list = ls())
library(dplyr)
library(stringr)
########################################################################################################################################################################################################
# args <- commandArgs(TRUE)
# chrom.name <- args[1]
# par.name <- args[2]
########################################################################################################################################################################################################
par.name.vec <- c("1863", "1909", "1950", "2048", "2066", "2283", "4593", "2365", "2393", "2515", "2572", "2683", "6909", "7073" )
chrom.name.vec <- c( "Chr01", "Chr02", "Chr03", "Chr04", "Chr05", "Chr06", "Chr07", "Chr08", "Chr09", "Chr10", "Chr11", "Chr12", "Chr13", "Chr14", "Chr15", "Chr16", "Chr17", "Chr18", "Chr19")
linkage.grp.names <- c("LG-I", "LG-II", "LG-III", "LG-IV", "LG-V", "LG-VI", "LG-VII", "LG-VIII", "LG-IX", "LG-X"," LG-XI", "LG-XII", "LG-XIII", "LG-XIV", "LG-XV", "LG-XVI", "LG-XVII", "LG-XVIII", "LG-XIX")
genetic.map.input.path <- "./onemap/bin_markers/genetic_maps/"
rippled.genetic.map.input.path <- "./onemap/bin_markers/genetic_maps/rippled_maps/"
final.genetic.map.output.path <- "./onemap/bin_markers/genetic_maps/finalized_maps/"
final.combined.genetic.map.output.path <- "./onemap/bin_markers/genetic_maps/finalized_maps/combined_maps/"
########################################################################################################################################################################################################
for (par.name in par.name.vec) { # par.name <- "1863"  
  combined.gen.map <- NULL
  for (chrom.name in chrom.name.vec) { # chrom.name <- chrom.name.vec[1]
    
    if (file.exists(paste0(rippled.genetic.map.input.path, chrom.name, "_", par.name, "_genetic.map"))) {
      gen.map <- read.table(file = paste0(rippled.genetic.map.input.path, chrom.name, "_", par.name, "_genetic.map"), header = FALSE, stringsAsFactors = FALSE)
    } else {
      gen.map <- read.table(file = paste0(genetic.map.input.path, chrom.name, "_", par.name, "_genetic.map"), header = FALSE, stringsAsFactors = FALSE)
    }
    
    string <- paste0(chrom.name, "_")
    physical.position <- gen.map[,2] %>% str_replace(string, "") %>% as.numeric()
    gen.map <- gen.map[, -1]
    gen.map <- cbind(gen.map, physical.position)
    colnames(gen.map) <- c("Marker_name", "Genetic_map_position (cM)", "Physical_map_position (bp)")
    
    # producing a combined genetic map format for Hudson-Alpha based on individual maps for the linkage groups
    combined.gen.map <- rbind(combined.gen.map, gen.map)
    
    write.csv(gen.map, paste0(final.genetic.map.output.path, chrom.name, "_", par.name, "_genetic.map"), quote = FALSE, row.names = FALSE)
  }
  write.csv(combined.gen.map, paste0(final.combined.genetic.map.output.path, par.name, "_genetic.map"), quote = FALSE, row.names = FALSE)
}



# Writing all the half-sib maps on to a single format
all.gen.maps <- NULL
for (par.name in par.name.vec) { # par.name <- "1863"  
  combined.gen.map <- NULL
  for (chrom.name in chrom.name.vec) { # chrom.name <- chrom.name.vec[1]
    
    if (file.exists(paste0(rippled.genetic.map.input.path, chrom.name, "_", par.name, "_genetic.map"))) {
      gen.map <- read.table(file = paste0(rippled.genetic.map.input.path, chrom.name, "_", par.name, "_genetic.map"), header = FALSE, stringsAsFactors = FALSE)
    } else {
      gen.map <- read.table(file = paste0(genetic.map.input.path, chrom.name, "_", par.name, "_genetic.map"), header = FALSE, stringsAsFactors = FALSE)
    }
    halfsib.fam.name <- paste0("GW-", par.name)
    linkage.grp <- linkage.grp.names[chrom.name.vec %in% chrom.name]
    string <- paste0(chrom.name, "_")
    physical.position <- gen.map[,2] %>% str_replace(string, "") %>% as.numeric()
    gen.map <- gen.map[, -1]
    gen.map <- cbind(halfsib.fam.name, linkage.grp, gen.map, physical.position)
    
    # producing a combined genetic map format for Hudson-Alpha based on individual maps for the linkage groups
    combined.gen.map <- rbind(combined.gen.map, gen.map)
  }
  all.gen.maps <- rbind(all.gen.maps, combined.gen.map)
}
colnames(all.gen.maps) <- c("Half-sib_family_name", "Linkage_group", "Marker_name", "Genetic_map_position (cM)", "Physical_map_position (bp)")
write.csv(all.gen.maps, paste0(final.combined.genetic.map.output.path, "7x7_all_14_combined_halfsib_genetic.map"), quote = FALSE, row.names = FALSE)
########################################################################################################################################################################################################
# END
########################################################################################################################################################################################################